...
namespace BasketLeague.Models
{
    class Team
    {
        ...
        public int Atacar(Team rival)
        {
            Random rnd = new Random();
            return 80 + (rnd.Next(5, 10) * Ataque + rnd.Next(5, 10) * Tiro) - (rnd.Next(1, 5) * rival.Defensa + rnd.Next(1, 5) * rival.Rebote);
        }

        public int Defender(Team rival)
        {
            Random rnd = new Random();
            return 30 + (rnd.Next(5, 10) * Defensa + rnd.Next(5, 10) * Rebote) - (rnd.Next(1, 5) * rival.Ataque + rnd.Next(1, 5) * rival.Tiro);
        }

        public int Resultado(int defender, int atacar)
        {
            Random rnd = new Random();
            return rnd.Next(defender, atacar);
        }
